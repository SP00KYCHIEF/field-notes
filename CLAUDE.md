...
Six formats: five print templates (`buildPagesZine` / `buildFullBleed` / `buildContact` / `buildMiniZine` / `buildBooklet`) plus **flip-through** (`buildFlipZine`), a page-turning *web* version — the digital twin of the print flow. All share the same building blocks (`doc`/`bar`/`coverBlock`/`metaRows`/`imgURL`) and one logical reading order (page 1 = cover, then images chronologically). `pages`, `full-bleed`, and `contact sheet` are simple sequential page flows (DOM order == reading order); `mini-zine` (PocketMod 8-panel single-sheet, one cut) and `booklet` (saddle-stitch) have real imposition math that rearranges those pages onto physical sheets — **don't touch it without re-verifying page numbers**; flip-through renders leaves straight in reading order (single-leaf CSS `rotateY` turn, vanilla JS, no library). The **auto cover** toggle makes `coverBlock` add a representative hero image (most-colorful reference) + a palette strip aggregated over the selection (`listPalette`/`heroItem`); off = the plain text cover. Print covers can also carry a **QR** (`coverBlock`'s `qr` arg) that scans back to the digital flip-through.

Three export paths, all from the same built HTML: **create zine** (`/api/zine` → open/print), **save .html** (`/api/zine-file` → `inlineZine` self-contained download, images/fonts inlined as `data:` URIs — so "save .html" on a flip zine yields one offline, double-clickable page-turning file), and **save .pdf** (`/api/zine-pdf` → real PDF via headless Chrome `--print-to-pdf`, `renderPDF`). The PDF path renders a **self-contained `file://` copy** (via `inlineZine`) — *not* the live `/zine` URL — because pointing headless Chrome back at this same server makes it fetch its own subresources and then hang without exiting. And it **polls for the finished PDF** (stable size + trailing `%%EOF`) rather than waiting for Chrome to exit, since headless Chrome reliably writes the file but often won't terminate cleanly. Chrome's `--user-data-dir` must live in a **short path** (temp dir, not the deep `.cache`): its unix-domain `SingletonSocket` overflows macOS's ~104-char socket-path cap otherwise and Chrome hangs. Degrades gracefully: no Chrome (or a render failure) returns `{ok:false, reason}` JSON instead of a PDF, and the client falls back to the browser print flow.
